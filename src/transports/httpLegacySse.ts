...
export function registerLegacySseRoutes(options: LegacySseRouteOptions): void {
  const { app, logger, createServer, sessions, validateRequest } = options;

  app.get(["/mcp/sse", "/sse"], async (req: Request, res: Response) => {
  ...
  app.post(["/mcp/messages", "/messages"], async (req: Request, res: Response) => {
    if (!validateRequest(req, res)) {
      return;
    }
