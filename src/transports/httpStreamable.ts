...
export async function runStreamableHttp(options: RunStreamableHttpOptions): Promise<RunningHttpServer> {
  ...
  const requestGuards: RequestHandler[] = [
    createRequestRateLimitMiddleware(requestRateLimiter),
    createOriginMiddleware(env.mcpAllowedOrigins, logger),
    createBearerAuthMiddleware(env.mcpServerBearerToken),
  ];
